...
def matcher(original=[], lookup=[], k_matches=5, ngram_length=3):
    ...
    matches_all_lookup = ['Lookup '+str(i+1) for i in range(0, k_matches)]
    all_matching = []
    for j in range(matches.shape[0]):
        all_matching.append(list(matches[matches_all_lookup].iloc[j]))
    matches['all_mathing'] = all_matching
    return matches
